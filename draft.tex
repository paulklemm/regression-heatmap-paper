...
\title{Regression Cube Analysis of Population Study Data}
...
\shortauthortitle{Klemm \MakeLowercase{\textit{et al.}}: Regression Cube Analysis of Population Study Data}
...
%%
The 2D heatmap \cite{Klemm2014VIS} highlights the pairwise correlation between features without a target disease by depicting the \emph{Cram\'{e}r's V} contingency values.
%%
...
\end{enumerate}
%%
\section{Regression Cube Analysis of Population Study Data}
\magenta{
...
%%
The idea emerged from analysis sessions of a previous project, which contained a 2D heatmap showing pairwise feature correlations based on \emph{Cram\'{e}r's V} contingency values \cite{Klemm2014VIS}.
%%
...
\begin{figure}[htb]
 ...
 \caption{
 (a) Overview visualization using a 2D heatmap for the formula $Z \sim X + Y$, where $Z$ assumes the feature $age$.
 %%
 ...
 %%
 (b) Now, $Z$ is set to all features $n$ and yields $n$ 2D heatmap visualizations.
 %%
...
%%
As described in our previous work \cite{Klemm2014VIS}, correlation values scaled between 0 (no correlation) and 1 (perfect correlation) can be encoded with color in a 2D heatmap.
%%
...
%%
We can apply the same 2D heatmap by translating the $R^2$ values to \emph{color saturation} (Fig.~\ref{fig:Cube}a).
%%
...
\begin{figure}[htb]
 ...
 %%
 [\textbf{4.DD}] Details about features using the 2D heatmap representation yield insights and hypotheses about feature relations.
 }
  \label{fig:Workflow}
\end{figure}
%%
...
\textbf{Zoom, Filter and Analyze Further} [\textbf{3.ZF}]. The slices of interest can then be analyzed using the 2D heatmap of the slice.
...
%%
Observations in the 2D heatmap or simply the CFS-based features can trigger new analyses by adjusting the underlying regression formulas.
%%
...
%%
The absolute difference in the regression metric values with the second cube is calculated, yielding a difference cube showing only the differences between the two formulas.
%%
...
\begin{figure}[htb]
 ...
 %%
 (c) 2D heatmap visualization of the selected slice for feature \emph{Pain/Discomfort}.
 %%
 }
  \label{fig:System}
\end{figure}
...
\begin{itemize}
	...
	%%
	\item The \emph{cube visualization} consisting of the 2D heatmap as well as a 3D representation of the whole cube \add{with facilities to change the represented regression metric and its range} [2.SI].
	%%
...
%%
Since all cells in the cube are represented using regression metric values, a difference cube is calculated by denoting the absolute difference of regression metric for each cell.
%%
...
%%
Figure~\ref{fig:Cube} shows that all values are mirrored along the diagonal of the 2D heatmap matrix.
%%
...
%%
It also gives context information about adjacent data values when using the 2D heatmap.
%%
...
%%
\magenta{Variables are ordered the same way in the 2D heatmap and 3D cube to preserve the mental model and make them visually analogous.}
...
%%
\paragraph{2D heatmap Plot Slice Visualization [4.DD].}
%%
The 2D heatmap (Fig.~\ref{fig:System}c) shows all values below the matrix diagonal of the current slice.
%%
...
%%
The 2D heatmap is implemented using \texttt{Data driven Documents} (\texttt{D3.js}), which is popular in information visualization using vector graphics \cite{D3}.
%%
...
\begin{figure*}[htb]
 ...
 %%
 A high correlation between \emph{Interleukin-6}, \emph{hepatic steatosis}, \emph{GGT} and \emph{Lipase} (highlighted using arrows) was revealed during the analysis using the 2D heatmap.
 %%
...
%%
Analyzing the 2D heatmap [\textbf{4.DD}] revealed high correlations for somatometric features, hepatic steatosis indicator features as well as laboratory values, such as \emph{creatinine} (used as renal retention parameter) and \emph{uric acid} (among others used as gout and diabetes risk factors) magnitudes.
%%
...
%%
These can then be analyzed further using the 2D heatmap for each cube slice.
%%

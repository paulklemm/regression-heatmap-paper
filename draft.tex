...
%%
Their main goal is to assess the pairwise influence of independent features towards the target feature.
%%
This is achieved with a plot matrix displaying models as contours towards the target feature.
%%
%
They also incorporate 3D visualizations for each pairwise combination, 
%...  zu umgangssprachlich: but mainly because it is popular with the engineers in their target domains.
%... lieber so?
but without any additional statistical information.
%%
Linked views of model deviations allow to select outliers.
%%
%... ein bisschen zusammengefasst, da imho zu detailliert.
Comparison of models is also included by mapping model values on line-style (e.g., solid or dashed) which limits the method to comparing only a few models at once.
%%
Furthermore, the plot matrix representation gets too complex and harder to understand with increasing number of independent features.
%%
...
%%
Sub-groups are automatically created by applying \emph{analytics} and expert-guided \emph{views}.
%%
They do not go into detail w.r.t the analytics used. They also seem to not incorporate confounder analysis (other than dividing groups by gender) and the user study lacks concrete goals.
%%
Their data differs strongly from ours, having multiple moments for each subject, yielding an approach focused on finding pathways of interest.
%%
...
%%
We implement this by supporting both: hypothesis-based and hypothesis-free analyses.
...
%%
Their proposed engine represents multiple heterogenous cohort study data sets as multiple normalized data cubes. Thus, data redundancy can be reduced and runtime measure aggregation can be carried out when measures of the different cubes have to be combined.
%%
...
%%
In contrast, we use regression models and associated $R^2$ values to depict relationships between a target feature and multiple independent features instead of one and employ the information into one global visualization.
%%
Also, we aim at a fast conducting large scale correlation analysis between many features to derive insight into the data.
...
%%
They recommend an opening up analysis system to a diverse target group, e.g., including patients, who can also observe information about their own lifestyle and associated risks.
%%
...
%%
They derive hypotheses using their high variation average message density algorithms, steering the user's focus on particular events.
%%
...
%%
In contrast to their method and other linked-view analysis based approaches, we show abstract information derived from a preceding analyze step to highlight relationships rather than displaying the actual data with various information visualization techniques.
...
%%
The techniques are useful for selected features or a data set with a small number of features due to their increase in complexity with every additional parameter and are therefore only suitable for small epidemiological data sets.
%%
...
%%
The dimensions for the calculations are fixed. In contrast, we assess different features for each model calculation and can therefore not benefit from a hierarchical calculation approach.
...
%%
Therefore, we utilized semiautomatic detection algorithms to analyze the lumbar spine shape \cite{Klemm2013VMV}.
%%
The presented models focussed on the lumbar spine canal and its shape, which was approximated and described with a line representation.
%%

...
%%
The abstract description allows to sort approaches to find works with similar problems to derive new design ideas and evaluate ideas.
%%
Based on their input parameter classification, we are visualizing \emph{model parameters} based on \emph{environmental parameters} in a \emph{global-to-local} navigation strategy.
%%
We solve a \emph{fitting} task by aiming to find models well suited for describing the input data.
}

\magenta{
Guo et al.~\cite{Guo} presented a multi-space visualizations to find linear relationships in the data with focus on extracting groups, which fit best.
%%
...
%%
The paper is focused on finding and tuning a model for a specific relationship, while we search for models w.r.t. all combinations in the data set.
}

\magenta{
M\"{u}hlbacher et al.~\cite{Muehlbacher} provide a framework for qualitative analyses of relationships and ranking features for numerical target features with regression models.
%%
...
%%
We employ the same steps by assessing single-relationships with the target and providing a overview over combinations.
%%
M\"{u}hlebacher et al. focus on a smaller number of features, which can be assessed in more detail, yielding a plot-matrix view, while we cover more features by abstracting the models using color coded regression metrics.
%%

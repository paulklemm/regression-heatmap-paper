...
%%
A multiple view system links spatial with non-spatial data and correlation coefficients are visualized using Kiviat diagrams (similar to star plots), correlations are plotted using p-values in the other views.
%%
...
\add{
%%
\emph{Adjusted} $R^2$ can be represented in the same way, since they share the scale.
%%
...
%%
\add{The selected metric (per default set to $R^2$)} of each heat map entry (\emph{voxel}) is mapped to opacity to reduce the overlap.
%%

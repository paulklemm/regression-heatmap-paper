...
\keywords{Interactive Visual Analysis, regression, Epidemiology, Breast Cancer, Hepatic Steatosis}
...
\begin{figure}[htb]
 ...
 %%
 The web-server (right) is written using \emph{Node.js} and hosted on \emph{Heroku}.
 %%
...
\texttt{Twitter Bootstrap} handles the page layout and provides a rich set of user interface elements.
%%
The 2D heat map is implemented using the \texttt{D3.js}~\cite{D3} information visualization library.
%%
...
The 3D plot is created using the WebGL-based library.
%%
...
The web-server is written in \texttt{Javascript} using \texttt{Node.js}, running on Googles V8 Javascript runtime environment.
%%

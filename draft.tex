...
%%
% der Satz kann raus. 1. Teil ist wirklich "common sense", 2. Teil wird im nÃ¤chsten Satz mit "...closest to ours" nochmal genannt.
...
%%
Generalized Pairs plots (\texttt{GPLOMÃÂ´S}) extend the idea of scatter plot matrices by the pairwise depiction of heterogeneous data using type-combination-dependent visualizations \cite{GPLOMS, Francois2013}.
%%
...
%%
Their research agenda acts as guideline for our design, comprising perceptual tuning towards human pattern recognition of important aspects, scalability between different data set sizes and application evaluation with domain experts.
%%
%% TODO: Use this one? "Statistical resilience of random populations to random perturbations" - http://www.ncbi.nlm.nih.gov/pubmed/19256997 - wie eine Variable sich ÃÂ¤ndert, wenn man per Zufall die Populationen verÃÂ¤ndert, um zu schauen wie zuverlÃÂ¤ssig diese Statistik ist.
...
Niemann et al. \cite{Niemann2014} investigate risk factors of hepatic steatosis (fatty liver disease) using decision trees with interactive data mining tools.
%%
They extracted association rules that serve as basis for a proof-of-concept evaluation.
%%
...
%%
%\com{GlaÃÂer, S., Niemann, U., Preim, B., and Spiliopoulou, M. (2013). Can we Distinguish Between Benign and Ma- lignant Breast Tumors in DCE-MRI}
...
%% 
% erst die ErklÃ¤rung, dann die Folge daraus =)
...
%%
% Verben in Überschriften immer groß
...
% -Dependent & -Based in Überschriften groß, sicherheitshalber nochmal Steffi fragen
% oder: Dimension Reduction Depending on the Target Variable
...
%%
The correlation-based feature selection (CFS) \cite{CFS} aims to find a feature subset that maximizes the \emph{merit value} $M_F$ which is the ratio between the average feature-class and feature-feature dependencies of features in a given feature set $F$.
%%
...
%%
% Übergang ist recht holprig gewesen
...
\\\\
We use the squared bracket abbreviation for each step to denote the affiliation in the system design section later on.
%%
...
%%
Figure~\ref{fig:Application} (right) shows a comparison cube with subtracted effects of age ($Z \sim Age$).
%%
...
\emph{TI}, a statistician with focus on epidemiology (8 years of experience) assesses the statistical reliability of the tool and the underlying methods without a focus on a specific data set.
%%
...
%%
Liver fat concentrations of no more than 10\% are mapped to the 'negative' class; values greater than 10\% are mapped to the 'positive' class.
%%
The data set contains labels for 578 participants for which the MRI data was available to that point.
%%
The MRI scans for each subject were introduced in SHIP-2, so the target feature is present for just one of three study waves.

%%
Apart from the target feature, the data set contains 199 features, comprising socio-demographic features (gender, age), consumption behavior (e.g. alcohol or tobacco), laboratory data (e.g. sera concentrations), and two features depicting the liver ultrasound.
%%
...
In \cite{Niemann2014}, the authors show different class distribution of liver fat concentrations of women and men.
%%
For women a association between age and liver fat was identified.
...
% \end{itemize}
%%
The latter were derived using manual segmentation of MRI image data for each subject carried out by radiologists.
%%
%%
...
%%
\textbf{3.ZF} by slicing through the cube using the mouse input to inspect the hotspots using the mosaic plot (\textbf{4.DD}) revealed high correlations for somatometric features, hepatic steatosis indicator-features as well as laboratory values, such as creatinine (used renal retention parameter) and uric acid (among others used as gout and diabetes risk factors) magnitudes.
%%
...
%%
Correlation hot spots matched with the ones from the dichotomous target, but were generally lower ($R^2$ of $0.37$ for somatometric features as opposed to $0.58$).
%%
...
%%
For the first time, they were able to derive a overview visualization custom tailored to underlying assumptions.
%%
...
%%
A accompanying search for correlations potentially highlighting confounders can be carried out using our method.
%%
...
%%
Findings and hypotheses derived through overview visualizations, however, are met with more confidence, because the user actually observed the behavior himself.
%%
...
%%
Modeling expert knowledge using regression formulas allow for \emph{hypothesis-based} investigation.
%%
...
\acknowledgments{SHIP is part of the Community Medicine Research net of the University of Greifswald, Germany, which is funded by the Federal Ministry of Education and Research (grant no. 03ZIK012), the Ministry of Cultural Affairs as well as the Social Ministry of the Federal State of Mecklenburg-West Pomerania. Whole-body MR imaging was supported by a joint grant from Siemens Healthcare, Erlangen, Germany and the Federal State of Mecklenburg-Vorpommern. The University of Greifswald is a member of the Ã¢ÂÂCentre of Knowledge InterchangeÃ¢ÂÂ program of the Siemens AG. This work was supported by the DFG Priority Program 1335: Scalable Visual Analytics.}
% \end{small}
\clearpage
\newpage

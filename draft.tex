...
%%
Explorative analyses and overview visualizations of the data set as presented in prior work \cite{Klemm2014VIS} are not tailored to a specific target variable and mostly highlight correlations between variables, which are known to the domain expert (e.g., correlation between body size and spine shape).
%%
...
%%
%We focus on cohort studies are a major epidemiological tool for gathering epidemiological features.
...
%%
The feature space comprises information about lifestyle, somatometric variables, medical parameters, genetic data as well as medical images.
%%
...
%Confounding features have to be considered in epidemiological analysis, as they influence both the exposure as well as the outcome of an analysis.
%%
...
\end{equation}
%%
A example of a regression formula would be $KidneyDisorder \sim Smoking + Obesity$.
...
%%
\emph{Nagelkerke $R^2$} behave different than $R^2$ values extracted from linear regression model.
%%
...
%%
They also incorporate 3D visualizations for each pairwise combination, but mainly because its popularity with their target domain engineers.
%%
...
%%
The main difference is their focus on analyzing one complex model in detail, yielding extensive plots, while we analyze a large amount of simpler models towards \emph{different} features.
%%
...
%% einheitlich 2D-XXX oder 2D XXX, meist ist es 2D XXX
...
The presented challenges in this application comprise the systems to be sufficiently comprehensible and intuitive to fit into the short time span clinicians have when analyzing data.
%%
...
Bertini et al. \cite{Bertini} present an overview over quality metrics describing high dimensional data.
%%
...
%%
%% TODO: Use this one? "Statistical resilience of random populations to random perturbations" - http://www.ncbi.nlm.nih.gov/pubmed/19256997 - wie eine Variable sich √É¬§ndert, wenn man per Zufall die Populationen ver√É¬§ndert, um zu schauen wie zuverl√É¬§ssig diese Statistik ist.
%% The Sparse Regression Cube: A Reliable Modeling Technique for Open Cyber-physical Systems
Ahmadi et al. \cite{Ahmadi} define the \emph{Sparse Regression Cube}, which partitions sparse high-dimensional data into subspaces, which are then described by their most reliable linear regression model.
%%
...
%%
They analyze subsets using a drilling-down approach by incorporating scatter plot matrices (\texttt{SPLOMÂ«S}) of quality metrics. %describing the features.
%%
...
%%
They extracted association rules that serve as basis for our proof-of-concept tests.
%%
...
%%
%\com{Gla√É¬üer, S., Niemann, U., Preim, B., and Spiliopoulou, M. (2013). Can we Distinguish Between Benign and Ma- lignant Breast Tumors in DCE-MRI}
...
%% 
% erst die Erkl√§rung, dann die Folge daraus =)
...
%%
% unified -> singular [Steffi?]
A regression model is based on expert knowledge, there is no singular rule how to apply them to a given set of features. Thus, they have to be applied with care.
%%
...
%%
% hours -> h da ms ja auch benutzt wird.
When each regression takes about 50~ms calculation, we have to roughly wait 14~h for the calculation to complete.
%%
...
%%
In general, many of them exhibit low or none correlation at all towards the target feature.
%%
...
%%
% nochmal checken wo $F$ eingef¸hrt wird, hier oder im ¸bern‰chsten Satz?
...
%%
% Ggf. wurde $F$ schon oben eingef¸hrt, TODO
...
%For features, which affect the most other features (Recall "Confounder" in Section~\ref{sec:EpidemiologicalData}), e.g., age or gender,  the number is significantly larger (about half the features in the whole data set).
%%
...
%%
% Adverb immer vors Hauptverb oder Satzende
...
\begin{figure}[htb]
 ...
 %%
 [\textbf{1.AF}] The workflow starts by declaring a formula regarding to specify a hypothesis, or use a predefined formula for a hypothesis-free analysis.
 %%
...
%%
% ‹bergang ist recht holprig gewesen
Our Regression Cube models is well-suited for different workflow analysis techniques, based on the Visual Analytics (VA) Mantra of Keim et al. \cite{Keim}:
...
%%
Using the operators, the dynamic variables and the dataset features, many different assumptions can be expressed.
%%
...
% warum war da ein "either", sollte der Satz etwas anderes ausdr¸cken?
...
\begin{figure}[htb]
 ...
 %\com{Also include active tooltip}
  ...
  \label{fig:System}
\end{figure}
%% scientific english: kurz und langweilig ;)
...
%%
Although this seems obvious, many cohort study data sets encode categorical features using ID-values, which are denoted in a data dictionary.
%%
...
%%
% TODO: p values kurz nennen? woher kommen die confidence intervals?
...
\begin{figure}[htb]
 ...
 \caption{
 The front-end (left) is realized with \emph{HTML5}/\emph{CSS3}/\emph{Javascript} and different Javascript libraries, such as \emph{Angularjs}, \emph{Three.js} and D3js.
 %%
...
%% TODO: nochmal Hinweis auf High dimensionality of data?!
...
%%
% UI muss mal erkl�rt ewrden, tauchte schon weiter vorn im Paper auf
...
%%
We conducted a three with two participants. %, who also co-authored this publication.
%%
...
%%
% ok?  to indicate absence or presence of the disease.
...
%%
%The acquisition wave is denoted using the appendix; 85 for S0 (SHIP-0), 55 for S1, 55 for S2 and 10 time-independent SNPs (DNA base pairs).
%%
...
%%
% ist das richtig? nicht female<52,male, female>52?
...
%% TODO Ergebnisse nachfragen: das ist doch super. Hier noch ein Satz ob die Radiologen das gut finden. ist das nicht richtig geil? Mehr Infos!!!
...
%%
During the analysis, one hot spot was always observable in the \emph{SI} and \emph{ZF} incorporating a high \emph{Interleukin-6} (\emph{IL-6}) correlation with liver fat values (Fig.~\ref{fig:Application} b).
%%
...
%%
At first, she was interested in correlations with the parenchyma tissue segmentations, which was selected through the drop-down for the z-axis (\textbf{2.SI}).
%%
She observed expected strong correlations with age, body fat percentage, hip and waist circumference
as well as menstrual period or pregnancy status.
%%
Fat women also have a larger breast fat percentage, which also correlates with other somatometric features.
%%
Age is a strong influencing factor, as breast tissue degrades over time and subsequently the parenchyma tissue.
%%
\emph{KH} proceeded using [\textbf{2.SI}] and [\textbf{3.DD}] to check for relationships for different target features, such as current hormone replacement therapy, BI-RADS (classification of the mammography findings) as well as different diseases, such as diabetes or gout.
%%
She observed relationships matching her expectations and expert knowledge.
%%
One unexpected relationship was observed between mammography lesions and menstruation cycle towards spiral contraception were observed ($R^2$ of $0.77$).
%%
\emph{KH} proceeded with a detailed analysis of the parenchyma tissue.
%\emph{KH} derived at the hypotheses: \emph{Somatometric features impose the major influences on the parenchyma tissue.}
%%

\textbf{Detailed Breast Parenchyma Analysis.}
%%
The analysis was conducted by calculating the cube $Parenchyma-Percentage \sim X + Y + Z$.
%%
Using the 3D cube, \emph{KH} observed several hotspots.
%%
Navigating to them using the slicing facility of the 3D visualization highlighted several features of high influence, such as image-derived features, as glandular tissue density, parenchyma segmentation metrics.
%%
Also, strong correlations were observed in the diabetes slice, confirming expectations of \emph{KH} w.r.t. to its strong influence to the parenchyma tissue.
%%
A surprising find were the strong correlations with kidney disorder ($R^2$ values around $0.9$).
%%
The \textbf{3.DD} analysis however showed only 8 subjects with kidney disorder.
%%
The finding was noted and investigated further in the near future using a extensive data set.
%%
Lastly \emph{KH} assessed the influence of contraception related features, such as use birth control pills or the spiral, but found no significant correlations with the parenchyma.
%%
Features, such as alcohol intake yield no elevated $R^2$ values.
%%
...
%%
%mamma anamnesis breast fed, contraception (spirale, pille), alcohol intake
%Lastly, \emph{KH} assessed the influence of image-derived features on the parenchyma tissue.
%%
%She observed expected results 
%Using the slicing mode to fast 
%Hypotheses
%Detailed analysis: Cholesterin, influence of image-derived feautres, classification of the BI-Rads (how well did the manual segmentation perform)
...
%%
In this context, \emph{TI} highlighted the ability to simultaneously analyze thousands of regression models while maintaining little time expenses for rating them.
%The novel way of modeling knowledge to derive hypothesis driven overview visualizations allow for detailed analysis of target features, either disease indicators or even image-derived features.
%%
%TI commented on the 
...
%%
%This incorporates detailed investigation of potential confounder features as well as looking for outlier subjects.
%%
...
%While the classical epidemiological workflow represents the foundation of the research in this area, explorative analysis based on the data gains importance.
...
%%
%Incorporating data mining algorithms with overview visualizations of statistical measures allow for reasoning about risk factors using underlying epidemiological data sets rather than predefined hypotheses.
%%
%We  a higher confidence of an expert in a result he or she actually observed in the data rather 
...
%%
Investigations of the the hepatic steatosis data set with both numerical and dichotomized liver fat values showed comparable results but slightly differing when it comes to details. %, but differences  %differing in detail.
%%
...
\acknowledgments{SHIP is part of the Community Medicine Research net of the University of Greifswald, Germany, which is funded by the Federal Ministry of Education and Research (grant no. 03ZIK012), the Ministry of Cultural Affairs as well as the Social Ministry of the Federal State of Mecklenburg-West Pomerania. Whole-body MR imaging was supported by a joint grant from Siemens Healthcare, Erlangen, Germany and the Federal State of Mecklenburg-Vorpommern. The University of Greifswald is a member of the √¢¬Ä¬òCentre of Knowledge Interchange√¢¬Ä¬ô program of the Siemens AG. This work was supported by the DFG Priority Program 1335: Scalable Visual Analytics.}
% \end{small}
\clearpage
\newpage

...
\begin{figure}[htb]
 ...
 \caption{
 (a) Overview visualization using a 2D heat map for the formula $Z \sim X + Y$, where $Z$ assumes the feature $age$.
 %%
...
%%
We can apply the same 2D heat map by translating the $R^2$ values to \emph{color saturation} (Fig.~\ref{fig:Cube}a).
%%
...
\begin{itemize}
	...
	%%
	\item The \emph{cube visualization} consisting of the 2D heat map as well as a 3D representation of the whole cube \add{with facilities to change the represented regression metric and its range} [2.SI].
	%%
...
%%
Figure~\ref{fig:Cube} shows that all values are mirrored along the diagonal of the 2D heat map matrix.
%%
...
%%
\magenta{Variables are ordered the same way in the 2D heat map and 3D cube to preserve the mental model and make them visually analogous.}
...
%%
These can then be analyzed further using the 2D heat map for each cube slice.
%%

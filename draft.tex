...
%%
%% TODO: Use this one? "Statistical resilience of random populations to random perturbations" - http://www.ncbi.nlm.nih.gov/pubmed/19256997 - wie eine Variable sich âÃÂ¬Â§ndert, wenn man per Zufall die Populationen verâÃÂ¬Â§ndert, um zu schauen wie zuverlâÃÂ¬Â§ssig diese Statistik ist.
...
%%
They analyze subsets using a drilling-down approach by incorporating scatter plot matrices (\texttt{SPLOMÃÂ«S}) of quality metrics. %describing the features.
%%
...
%%
%\com{GlaâÃÂ¬Ã¼er, S., Niemann, U., Preim, B., and Spiliopoulou, M. (2013). Can we Distinguish Between Benign and Ma- lignant Breast Tumors in DCE-MRI}
...
%% 
% erst die ErklâÂ§rung, dann die Folge daraus =)
...
%%
% nochmal checken wo $F$ eingefÂ¸hrt wird, hier oder im Â¸bernâ°chsten Satz?
...
%%
% Ggf. wurde $F$ schon oben eingefÂ¸hrt, TODO
...
%%
% â¹bergang ist recht holprig gewesen
...
% warum war da ein "either", sollte der Satz etwas anderes ausdrÂ¸cken?
...
%%
% UI muss mal erklärt ewrden, tauchte schon weiter vorn im Paper auf
...
%%
\emph{KH} proceeded using \textbf{2.SI} and \textbf{3.DD} to check for relationships for different target features, such as current \emph{hormone replacement therapy}, \emph{BI-RADS} (classification of the mammography findings) as well as different diseases, such as \emph{diabetes} or \emph{gout}.
%%
...
%%
The analysis was conducted by calculating the cube $Parenchyma\_Percentage \sim X + Y + Z$.
%%
Using the 3D cube, \emph{KH} observed several hotspots.
%%
Navigating to them using the slicing facility of the 3D visualization highlighted features of high influence, such as image-derived features, as \emph{glandular tissue density} and \emph{parenchyma segmentation} metrics.
%%
...
%%
The \textbf{3.DD} analysis however showed only 8 subjects with \emph{kidney disorder}.
%%
...
%She noted that she usually would have to spend hours using \texttt{SPSS} to assess the data in the detail we presented.
%%
...
%%
%Our method combines powerful statistical techniques with 

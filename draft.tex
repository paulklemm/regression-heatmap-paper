...
%%
UN therefore started the calculation using the formula $mrt\_liverfat\_s2 \sim X + Y + Z$.
%%
...
The target feature derived from the MRI image data was discretized in \cite{Niemann2014} as decision trees algorithms require a categorical target feature.
%%
Since the \emph{regression cubes} analysis is not restricted in this way, UN applied the same analysis for \emph{males} and the two female group using its whole data range.
%%

Using the \emph{Regression Cube}, UN 
\textbf{Interleukin-6 correlation with liver fat.}

\com{ToDo: Reflect on the different methods - one is used to define subsets which define the target feature very well, the other looks how good the features describe the target itself}
\com{1. Can we reconstruct relationships based on the regression cube?}

\com{2. Does discretization of the numeric target feature influence the result?}
